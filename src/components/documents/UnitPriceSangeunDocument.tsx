...
interface UnitPriceSangeunDocumentProps {
    items: LineItem[];
}

export default function UnitPriceSangeunDocument({ items }: UnitPriceSangeunDocumentProps) {
    // 공종별로 그룹화하고 산근 정보 추가
    ...
    return (
        <div className="space-y-6">
            {Object.entries(groupedItems).map(([category, categoryItems]) => (
                <Card key={category}>
                    <CardHeader>
                        <CardTitle>{category} 산근</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="text-right">산근</TableHead>
